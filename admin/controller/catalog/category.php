...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		foreach ($options as $key => $option) {
			$$key = isset($this->request->get[$key]) ? $this->request->get[$key] : $option;
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'visibility'  => $this->url->link('catalog/category/visibility', 'user_token=' . $this->session->data['user_token'] . '&category_id=' . $result['category_id'] . $url, true),
		...
		foreach (array('page', 'store_id') as $key) {
			if (isset($this->request->get[$key])) {
				$url .= '&' . $key . '=' . $this->request->get[$key];
			}
		}
		foreach (array('name', 'sort_order') as $key) {
			$data['sort_'.$key] = $this->url->link(
				'catalog/category', 
				'user_token=' . $this->session->data['user_token'] . '&sort=' . $key . $url,
				true
			);
		}
		...
		// Pagination links
		$url = '';
		foreach (array('sort', 'order', 'store_id') as $key) {
			if (isset($this->request->get[$key])) {
				$url .= '&' . $key . '=' . $this->request->get[$key];
			}
		}
		...
		$data['stores'] = $this->model_setting_store->getStoreNames();
		foreach ($data['stores'] as &$store) {
			if ($store['store_id'] == $store_id) {
				$store['selected'] = ' selected="selected"';
			}
			$store['url'] = $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $store['store_id'] . $url, true);
		}
	...
	protected function getForm() {
		...
		} elseif(!isset($this->request->get['category_id']) && isset($this->request->get['store_id'])) {
			$data['category_store'] = $this->request->get['store_id'];
		} else {
	...
	public function visibility() {
		...
		$this->load->model('catalog/category');
		$status = $this->model_catalog_category->changeStatus($this->request->get['category_id']);
