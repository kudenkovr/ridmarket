...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$url_filters .= '&filter_name=' . urlencode(html_entity_decode($filter_name, ENT_QUOTES, 'UTF-8'));
		} else {
			$filter_name = '';
		}
		...
		if (isset($this->request->get['filter_model'])) {
			...
			$url_filters .= '&filter_model=' . urlencode(html_entity_decode($filter_model, ENT_QUOTES, 'UTF-8'));
		} else {
			$filter_model = '';
		}
		...
		if (isset($this->request->get['filter_price'])) {
			...
			$url_filters .= '&filter_price=' . $filter_price;
		} else {
			$filter_price = '';
		}
		...
		if (isset($this->request->get['filter_quantity'])) {
			...
			$url_filters .= '&filter_quantity=' . $filter_quantity;
		} else {
			$filter_quantity = '';
		}
		...
		if (isset($this->request->get['filter_status'])) {
			...
			$url_filters .= '&filter_status=' . $filter_status;
		} else {
			$filter_status = '';
		}
		...
		$data['import'] = $this->url->link('update/fora', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($data['stores'] as &$store) {
			...
			$store['url'] = $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $store['store_id'] . $url, true);
		}
