...
class ModelCatalogCategory extends Model {
	...
	public function changeStatus($category_id) {
		...
		$status = $query->row['status'] ? 0 : 1;
		$this->db->query('UPDATE ' . DB_PREFIX . 'category SET status="'.$status.'" WHERE category_id = "'.(int)$category_id.'"');
		return $status;
	}
	...
	public function getTotalCategories($data=array()) {
		...
		if (array_key_exists('store_id', $data)) {
			$sql .= " WHERE c2s.store_id = '" . (int)$data['store_id'] . "'";
		}
