...
class ModelSettingStore extends Model {
	...
	public function getStoreNames() {
		$store_data = $this->cache->get('store_names');

		if (!$store_data) {
			$query = $this->db->query("SELECT s.store_id, s.value AS name FROM " . DB_PREFIX . "setting s WHERE s.key='config_name' ORDER BY s.store_id");

			$store_data = $query->rows;

			$this->cache->set('store_names', $store_data);
		}
